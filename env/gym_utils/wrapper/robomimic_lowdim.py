...
class RobomimicLowdimWrapper(gym.Env):
    ...
    def reset(self, options={}, **kwargs):
        ...
        sim_state = options.get(
            "sim_state", None
        )  # probe helper: jump to a state saved by get_sim_state
        if sim_state is not None:
            raw_obs = self.env.reset_to({"states": np.asarray(sim_state)})
    ...
    def get_sim_state(self):
        """Probe helper: underlying robosuite mujoco state vector."""
