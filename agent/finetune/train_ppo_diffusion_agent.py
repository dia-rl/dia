...
class TrainPPODiffusionAgent(TrainPPOAgent):
    ...
    def run(self):
        ...
        resume_itr = int(self.cfg.train.get("resume_from_itr", 0)) if hasattr(self, "cfg") else 0
        if resume_itr > 0:
            self.load(resume_itr)
            self.itr = resume_itr + 1
            log.info(f"Resumed DPPO base from iter {resume_itr}, starting at iter {self.itr}")
        ...
        while self.itr < self.n_train_itr:
            ...
            for step in range(self.n_steps):
                ...
                _t_policy += _time.perf_counter() - _p0
            ...
            _t_roll = _time.perf_counter() - _t_roll0
            log.info(
                f"[TIMING] rollout={_t_roll:.2f}s | policy_fwd={_t_policy:.2f}s | "
                f"env_step={_t_env:.2f}s | other={_t_roll - _t_policy - _t_env:.2f}s "
                f"(n_steps={self.n_steps} n_envs={self.n_envs} "
                f"ft={self.model.ft_denoising_steps} eval={eval_mode})"
            )
            self._t_roll_last = _t_roll
            self._t_upd0 = _time.perf_counter()
            ...
            if not eval_mode:
                with torch.no_grad():
                    ...
                    for t in reversed(range(self.n_steps)):
                        if t == self.n_steps - 1:
                            nextvalues = nextvalues_boundary
                        else:
                ...
                returns_k = (
                    torch.tensor(returns_trajs, device=self.device)
                    .float()
                    .reshape(-1)
                )
                values_k = (
                    torch.tensor(values_trajs, device=self.device)
                    .float()
                    .reshape(-1)
                )
                advantages_k = (
                ...
                for update_epoch in range(self.update_epochs):
                    ...
                    for batch in range(num_batch):
                        ...
                        v_outer_loss = torch.tensor(0.0, device=self.device)
                        loss = (
                            pg_loss
                            + entropy_loss * self.ent_coef
                            + v_loss * self.vf_coef
                            + v_outer_loss * self.vf_coef
                            + bc_loss * self.bc_loss_coeff
                        )
                        clipfracs += [clipfrac]

                        # update policy and critic(s)
                        ...
                        if self.itr >= self.n_critic_warmup_itr:
                            actor_grad_norm = float(
                                sum(
                                    ...
                                    for p in self.model.actor_ft.parameters()
                        ...
                        log.info(
                            f"approx_kl: {approx_kl}, actor_gnorm: {actor_grad_norm:.6f}, update_epoch: {update_epoch}, num_batch: {num_batch}"
                        )
                ...
                y_pred = values_k.cpu().numpy().reshape(-1)
                y_true = returns_k.cpu().numpy().reshape(-1)
                var_y = np.var(y_true)
                explained_var = (
            ...
            if self.itr % self.log_freq == 0:
                ...
                if eval_mode:
                    ...
                    log.info(
                        f"[TIMING] update={_time.perf_counter() - self._t_upd0:.2f}s "
                        f"| rollout={getattr(self, '_t_roll_last', float('nan')):.2f}s "
                        f"(update = value/logprob + GAE + PPO epochs)"
                    )
                    if self.use_wandb:
                        wandb_metrics = {
