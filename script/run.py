...
def main(cfg: OmegaConf):
    ...
    # Provenance: which machine actually produced this run. Nothing else in the run
    # records it -- logdir is identical across clusters that share a filesystem -- so
    # without this the only trace is the SLURM stdout file, which gets cleaned up.
    # (Recovering where the transport runs executed proved impossible for that reason.)
    _prov = {
    ...
    log.info("PROVENANCE " + "  ".join(f"{k}={v}" for k, v in _prov.items()))
