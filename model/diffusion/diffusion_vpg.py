...
class VPGDiffusion(DiffusionModel):
    ...
    def step(self):
        ...
        """
        # anneal min_sampling_denoising_std (only if it's an annealing scheduler;
        # scalar or list-of-floats configs need no step).
        if callable(getattr(self.min_sampling_denoising_std, "step", None)):
    ...
    def get_min_sampling_denoising_std(self):
        if isinstance(self.min_sampling_denoising_std, (int, float)):
            return float(self.min_sampling_denoising_std)
        return float(self.min_sampling_denoising_std())
    ...
    def forward(
        ...
        return_chain=True,
        use_base_policy=False,
        perturb_at_chain_idx=None,
        perturb_at_chain_magnitude=0.0,
        commit_chain_idx=None,
    ):
        """
        ...
        for i, t in enumerate(t_all):
            ...
            # --- early-exit diagnostic (#1): commit the predicted clean action at step i ---
            # DDPM posterior mean mu = c1*x_recon + c2*x  ->  x_recon = (mu - c2*x)/c1.
            # Returns the best-guess clean action after (i+1) denoising steps; default
            # commit_chain_idx=None leaves the normal sampling path untouched.
            if commit_chain_idx is not None and i == commit_chain_idx:
                c1 = extract(self.ddpm_mu_coef1, t_b, x.shape)
                c2 = extract(self.ddpm_mu_coef2, t_b, x.shape)
                x_recon = (mean - c2 * x) / c1
                ...
                return Sample(x_recon, None)
            std = torch.exp(0.5 * logvar)

            cur_min_std = min_sampling_denoising_std
            ...
            if self.use_ddim:
                if deterministic:
                    ...
                    std = torch.clip(std, min=cur_min_std)
            else:
            ...
            # Inner-step perturbation: if we just computed x at chain index
            # `perturb_at_chain_idx`, add Gaussian noise of given magnitude.
            # The perturbation enters the chain AND propagates to subsequent
            # denoising steps (so the model gets a chance to "recover").
            if perturb_at_chain_idx is not None and perturb_at_chain_magnitude > 0:
                cur_chain_idx = None
                if not self.use_ddim and t <= self.ft_denoising_steps:
                    cur_chain_idx = self.ft_denoising_steps - int(t)
                ...
                if (
                    cur_chain_idx is not None
                    and cur_chain_idx == perturb_at_chain_idx
                ):
                    x = x + perturb_at_chain_magnitude * torch.randn_like(x)
    ...
    @torch.no_grad()
    def predict_x0(self, cond, x, t_diffusion):
        """Predicted clean action x_recon from latent x at diffusion timestep
        t_diffusion, using the fine-tuned actor. DDPM + predict_epsilon path
        (the dual-critic setup). Used by potential-based inner-reward shaping (#2)
        and the early-exit diagnostic. t_diffusion: (B,) long tensor."""
        noise = self.actor_ft(x, t_diffusion, cond=cond)
        if self.predict_epsilon:
            x_recon = (
                extract(self.sqrt_recip_alphas_cumprod, t_diffusion, x.shape) * x
                - extract(self.sqrt_recipm1_alphas_cumprod, t_diffusion, x.shape)
                * noise
            )
        else:
            x_recon = noise
        if self.denoised_clip_value is not None:
            x_recon = x_recon.clamp(
