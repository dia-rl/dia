"""Ablation: V_inner regressed onto an outer return kept in RAW reward units."""
import os
import math
import time
import einops
...
# Fallbacks only; every config in cfg/ sets these explicitly, so no shipped result changes.
Q_UPDATE_EPOCHS = 20
...
class TrainAblationOuterTargetRawAgent(TrainPPOAgent):
    def __init__(self, cfg):
        ...
        # Second value function on the raw reward, used only for the V_inner target.
        import hydra as _hydra
        self.critic_raw = _hydra.utils.instantiate(cfg.model.critic).to(self.device)
        self.critic_raw_optimizer = torch.optim.AdamW(
            self.critic_raw.parameters(),
            lr=float(cfg.train.critic_lr),
            weight_decay=float(cfg.train.get("critic_weight_decay", 0)),
        )
    ...
    def scale_match_inner(self, A_inner, advantages_outer):
        ...
        thing at every k. Overridden by the noise ablation in
        train_ablation_dia_noise_agent.py."""
        sigma_outer = float(advantages_outer.std()) + 1e-8
        sigma_inner = float(A_inner.std()) + 1e-8
        return A_inner * (sigma_outer / sigma_inner)

    # critic_raw hangs off the agent, not self.model, so save/load it explicitly (its optimizer already was).
    def save_model(self):
        super().save_model()
        path = os.path.join(self.checkpoint_dir, f"state_{self.itr}.pt")
        data = torch.load(path, weights_only=False)
        data["critic_raw"] = self.critic_raw.state_dict()
    ...
    def load(self, itr):
        ...
        data = torch.load(path, weights_only=False)
        if "critic_raw" in data:
            self.critic_raw.load_state_dict(data["critic_raw"])
            log.info("Restored critic_raw from checkpoint")
        else:
            log.warning(
                "Checkpoint has no critic_raw; it restarts from scratch while its "
                "optimizer state is restored, which perturbs this arm's V_inner target"
            )
    ...
    def run(self):
        ...
        # Bind before the loop: with reset_at_iteration=False a resumed run would hit the rollout unassigned.
        prev_obs_venv = None
        while self.itr < self.n_train_itr:
            ...
            if not eval_mode:
                ...
                # Q trains on the raw env reward, not the scaled one, so its target stays in return units.
                ...
                # ---- ABLATION: outer GAE return, in raw reward units ----
                with torch.no_grad():
                    vr = self.critic_raw(
                        {"state": obs_flat_for_q}
                    ).view(S, E).cpu().numpy()
                    boot_vr = self.critic_raw(obs_venv_ts).view(-1).cpu().numpy()
                adv_r = np.zeros((S, E))
                last_r = np.zeros(E)
                for t in reversed(range(S)):
                    nextv = vr[t + 1] if t < S - 1 else boot_vr
                    nonterm = 1.0 - terminated_trajs[t]
                    delta_r = (
                        reward_trajs[t] + self.gamma * nextv * nonterm - vr[t]
                    )
                    adv_r[t] = last_r = (
                        delta_r + self.gamma * self.gae_lambda * nonterm * last_r
                    )
                returns_r = adv_r + vr
                v_target = torch.tensor(
                    np.ascontiguousarray(returns_r.reshape(N)),
                    device=self.device, dtype=torch.float32,
                )
                # Fit critic_raw to the return it just produced, as model.critic tracks the scaled one.
                for _ in range(self.update_epochs):
                    perm_r = torch.randperm(N, device=self.device)
                    for st_r in range(0, N, self.v_inner_minibatch_size):
                        idx_r = perm_r[st_r:st_r + self.v_inner_minibatch_size]
                        pred_r = self.critic_raw(
                            {"state": obs_flat_for_q[idx_r]}
                        ).view(-1)
                        loss_r = ((pred_r - v_target[idx_r]) ** 2).mean()
                        self.critic_raw_optimizer.zero_grad()
                        loss_r.backward()
                        self.critic_raw_optimizer.step()
                ...
                # Inner GAE: δ_k = V[k+1] - V[k]; A[k] = δ_k + γ_in λ_in A[k+1]
                ...
                # LR step; the actor schedule stays gated on warmup so both agents keep the same actor LR.
                if self.itr >= self.n_critic_warmup_itr:
