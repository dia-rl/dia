...
class MultiStep(gym.Wrapper):
    ...
    def __init__(
        ...
        pass_step_rewards=False,
        time_limit_as_truncation=False,
        verbose=False,
        **kwargs,
    ):
        super().__init__(env)
        ...
        # `reward` returned by step() is the SUM over the action chunk, so it cannot
        # say which env step inside the chunk earned it. With this on, the per-env-step
        # rewards are also passed out. Off by default; nothing else changes.
        self.pass_step_rewards = pass_step_rewards
        # gym's TimeLimit sets info["TimeLimit.truncated"] = (not done) and THEN forces
        # done = True, so on that branch `done` is always True and upstream's
        # `terminated = done` marks a pure time-limit cutoff as a real termination. That
        # zeroes the value bootstrap (and DIA's Q target) at a state whose future is
        # worth a full episode of return. HalfCheetah never terminates naturally, so
        # every one of its episode ends takes that path. Setting this True treats the
        # cutoff as a truncation instead. Default False = upstream behaviour unchanged.
    ...
    def step(self, action):
        ...
        for act_step, act in enumerate(action):
            ...
            if "TimeLimit.truncated" not in info:
                ...
                if self.time_limit_as_truncation:
        ...
        if self.pass_step_rewards:
            # padded to the chunk length so the array shape is the same every step,
            # which the vector env's info aggregation needs
            sr = np.full(self.n_action_steps, np.nan, dtype=np.float32)
            sr[: len(self.reward)] = np.asarray(self.reward, dtype=np.float32)
            info["step_rewards"] = sr
