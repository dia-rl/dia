...
class RobomimicImageWrapper(gym.Env):
    ...
    @staticmethod
    def _to_chw_255(img):
        """Image observation as float C H W in [0, 255], the layout and range the
        ViT encoder expects.

        robomimic <0.4 returned float C H W in [0, 1]; 0.5 returns uint8 H W C
        (env_robosuite.get_observation only flips height). Both are accepted here so
        the wrapper does not depend on the robomimic version.
        """
        if img.shape[-1] == 3:  # H W C -> C H W
            img = np.moveaxis(img, -1, 0)
        if np.issubdtype(img.dtype, np.integer):
            return img.astype(np.float32)  # already [0, 255]
        return img.astype(np.float32) * 255.0  # [0, 1] -> [0, 255]

    def get_observation(self, raw_obs):
        obs = {"rgb": None, "state": None}  # stack rgb if multiple cameras
        for key in self.obs_keys:
            if key in self.image_keys:
                img = self._to_chw_255(raw_obs[key])
                if obs["rgb"] is None:
                    obs["rgb"] = img
                else:
