...
def make_dataset(env_name, save_dir, save_name_prefix, val_split, logger):
    ...
    for i, cur_index in tqdm(enumerate(done_indices), total=len(done_indices)):
        ...
        if np.sum(trajectory["rewards"]) > 0:
            ...
            # Guard degenerate dims: floor the per-dim range so ~constant dims (obs_max~obs_min)
            # don't divide by ~0, and clip the result. MUST match env wrapper normalize_obs.
            _NORM_RANGE_FLOOR, _CLIP_OBS = 0.1, 10.0
            _denom = np.maximum(obs_max - obs_min, _NORM_RANGE_FLOOR)
            trajectory["states"] = np.clip(
                2 * (trajectory["states"] - obs_min) / _denom - 1, -_CLIP_OBS, _CLIP_OBS
            )
            trajectory["actions"] = (
                2
